...
BehaviorState BehaviorSequence::Execute(Blackboard* pBlackBoard)
{
	...
	for(IBehavior* behavior : m_ChildBehaviors)
	{
		...
		//if a child returns Running:
		if(m_CurrentState == BehaviorState::Failure || m_CurrentState == BehaviorState::Running)
		{

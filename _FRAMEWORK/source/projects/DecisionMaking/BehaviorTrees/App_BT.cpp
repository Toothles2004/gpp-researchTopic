...
void App_BT::Start()
{
	...
	//BehaviorTree* pBehaviorTree = new BehaviorTree(pBlackboard, new BehaviorAction(BT_Actions::ChangeToPatrol));
		...
		({
			...
			({
				...
				({
					...
					new BehaviorSequence
					({
						new BehaviorNotConditional(BT_Conditions::ReachedSearchPoint),
						new BehaviorAction(BT_Actions::ChangeToChase)
					}),
...
Elite::Blackboard* App_BT::CreateBlackboard(SmartAgent* smartAgent)
{
	...
	pBlackboard->AddData("LastKnownPos", Vector2{});
	pBlackboard->AddData("MaxSearchTime", m_MaxSearchTime);
	pBlackboard->AddData("TimeLastSeen", seconds{});
	pBlackboard->AddData("HasReachedLastSeenPoint", bool{});

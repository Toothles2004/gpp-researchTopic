...
App_FSM::~App_FSM()
{
	...
	SAFE_DELETE(m_pTargetVisible);
	SAFE_DELETE(m_pTargetNotVisible);
	SAFE_DELETE(m_pSearchingTooLong);
}
...
void App_FSM::Start()
{
	...
	m_pTargetVisible = new ::FSMState::IsTargetVisible();
	m_pTargetNotVisible = new ::FSMState::IsTargetNotVisible();
	m_pSearchingTooLong = new ::FSMState::IsSearchingTooLong();

	//4. Create the finite state machine with a starting state and the blackboard
	FiniteStateMachine* pFiniteStateMachine = new FiniteStateMachine(m_pPatrolState, pBlackboard);
	...
	pFiniteStateMachine->AddTransition(m_pPatrolState, m_pChaseState, m_pTargetVisible);
	pFiniteStateMachine->AddTransition(m_pChaseState, m_pSearchState, m_pTargetNotVisible);
	pFiniteStateMachine->AddTransition(m_pSearchState, m_pChaseState, m_pTargetVisible);
	pFiniteStateMachine->AddTransition(m_pSearchState, m_pPatrolState, m_pSearchingTooLong);

	//6. Activate the decision making structure on the custom agent by calling the SetDecisionMaking function
	m_pGuard->SetDecisionMaking(pFiniteStateMachine);
}
...
Elite::Blackboard* App_FSM::CreateBlackboard(SmartAgent* smartAgent)
{
	...
	pBlackboard->AddData("Agent", m_pPlayer);
	...
	pBlackboard->AddData("TimerSearching", timerSearching);

	//pBlackboard->ChangeData();
	// Different function
